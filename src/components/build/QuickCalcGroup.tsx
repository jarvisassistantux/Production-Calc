      ...
      {!collapsed && (
        <div>
          {resolved.map(({ lineItem, equipment }) => {
            ...
            return (
              <div key={lineItem.id} className="flex items-center gap-2 px-4 py-2 border-b border-gray-800/30 hover:bg-gray-800/30 group text-sm">
                <div className="flex-1 min-w-0">
                ...
                <div className="w-24 text-right shrink-0">
                  <div className="font-mono text-amber-400 text-sm">{fmtWatts(equipment.watts * lineItem.quantity)}W</div>
                  <div className="text-xs text-gray-500">{(equipment.weightLbs * lineItem.quantity).toFixed(0)} lbs</div>
                </div>
                ...
                <div className="w-20 text-right shrink-0">
                  {equipment.watts > 0 ? (
                    <>
                      <div className="font-mono text-xs text-blue-400">{fmtAmps(unitAmps)}A</div>
                      <div className="text-[11px] text-gray-500">{Math.max(1, maxPerLine)}/line</div>
                    </>
                  ) : (
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {build.circuits.length > 0 && (
                    <select
                      ...
                      className="w-20 px-1 py-0.5 bg-gray-800 border border-gray-700 rounded text-[10px] text-gray-500 focus:outline-none sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
                  ...
                  <button
                    ...
                    className="text-gray-600 hover:text-red-400 min-w-[44px] min-h-[44px] flex items-center justify-center sm:min-w-0 sm:min-h-0 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
                    title="Remove"
                  >&#x2715;</button>
                </div>
              </div>
